...
class MessageMailer:
    ...
    @classmethod
    async def start_mailing(cls, message: Message, repository: Repository, delay: float=None) -> None:

        delay = delay or cls.DEFAULT_DELAY
        scope = await repository.user.get_all_users_id()
        ...
        for sent, user_id in enumerate(scope, 1):
            ...
            try:
                ...
                # try:
                #     await repository.user.update_user_active(user_id=user_id, active=False)
                # except:
                #     pass
...
class GroupMessageMailer:
    ...
    @classmethod
    async def start_mailing(cls,message: Message, bot: Bot, media: list, repository: Repository, delay: float=None) -> None:
